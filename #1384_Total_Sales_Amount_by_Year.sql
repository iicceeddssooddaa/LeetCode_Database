...
SELECT 
    ...
    average_daily_sales * 
        (CASE
            WHEN start_year = end_year THEN DATEDIFF(period_end,period_start)
            WHEN start_year < report_year AND report_year < end_year THEN DATEDIFF(STR_TO_DATE(CONCAT(report_year,'-12-31'),'%Y-%m-%d'), STR_TO_DATE(CONCAT(report_year,'-01-01'),'%Y-%m-%d'))
            WHEN start_year = report_year THEN DATEDIFF(STR_TO_DATE(CONCAT(report_year,'-12-31'),'%Y-%m-%d'), period_start)
            ELSE DATEDIFF(period_end, STR_TO_DATE(CONCAT(report_year,'-01-01'),'%Y-%m-%d'))
